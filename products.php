		...
		<div class="container-fluid" id="Not_Ajax_Output">
			...
			<?php
				...
				}else
				{
					...
					foreach ($pdo->query($sql) as $row) {
					    echo '<div class="col-4-lg product" id="' . $row['a.id'] . '">' . '<img src="data:image/jpeg/base64/' . base64_decode($row['b.image']) . '" ' . $row['a.name'] . ' ' . $row['a.description'] . ' ' . $row['a.cost'] . ' <a href="#">Add to Cart</a></div>';
					}
				}
			?>
